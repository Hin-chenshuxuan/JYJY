...
public class OrderController {
     ...
     * @param
     */
    @ApiOperation(value = "判断用户是否可下载滤镜商品",notes = "判断用户是否可下载滤镜商品",tags = {"OrderController"})
    @ApiImplicitParams({
            @ApiImplicitParam(paramType = "path", name = "userId", value = "用户ID", required = true, dataType = "Integer"),
            @ApiImplicitParam(paramType = "path", name = "filterId", value = "滤镜商品ID", required = true, dataType = "Integer")
    })
    @GetMapping(value = "/{userId}/{filterId}")
    public Result<Boolean> isDownLoad(@PathVariable Integer userId,@PathVariable Integer filterId){
        Boolean b = orderService.isDownload(userId, filterId);
        return new Result<Boolean>(true,StatusCode.OK,"查询成功",b);
    }
     ...
     * @return
     */
    @ApiOperation(value = "查询所有Order",notes = "查询所有Order方法详情",tags = {"OrderController"})
    @GetMapping
